### Jonathan Stern added loaded internal energies for PartType0 05/20/2021
...
class Snapshot:
    ...
    def __init__(self, filename):
        if os.path.isfile(filename):
            ...
            self.SnapshotData['energies'] = self.ProcessParticlesEnergies()
        else:
    ...
    # Energy processing
    def ProcessParticlesEnergies(self):
        nbytes = self.getRecordLength(self.binfile.read(4)) + 4
        body = self.binfile.read(nbytes)
        return self.unpackEnergies(body)

    def unpackEnergies(self, instring):
        fmtstring = "{0:d}f4x".format(self.Npart[0])
        everything = struct.unpack(fmtstring, instring)
        self.internalEnergies = np.array(everything[:self.Npart[0]])
        return self.internalEnergies
