...
static struct NODE *trees[6];       /* we construct one tree for each particle type */
static long    ntype[6];             /* holds the number of particles of each type */
static int    treecost[6];          /* sums the cell-particle interactions  (diagnostic) */
static int    treecost_quadru[6];   /* sums the cell-particle interactions  (diagnostic) */

static int    numnodestotal;        /* total number of nodes */
static int    numnodestree[6];     /* number of nodes of each particle type */
static int    numnodes;
static long int MaxNodes;
...
int force_treebuild(void **pospointer, long npart, double thetamax,int costresetflag)
   ...
   (different softening lengths) a different tree */
{
  ...
  printf("ntypes= %ld %ld %ld %ld %ld %ld\n",ntype[0],ntype[1],ntype[2],ntype[3],ntype[4],ntype[5]);
...
/* packs the particles of the correct type in a BH-tree */
{
  ...
  for(i=fp+1;i<N;i++)  
      ...
      while(1)
	{
      ...
      if((i%10000)==0) printf("check 3+4 i=%d \n",i);
...
void force_treeallocate(long int maxnodes)  /* usually maxnodes=2*npart is sufficient */
{
  long int bytes;
  ...
  if(!(nodes=malloc(bytes=MaxNodes*sizeof(struct NODE))))
    {
      printf("failed to allocate memory for %ld tree-nodes (%ld bytes).\n",MaxNodes,bytes);
      exit(3);
    }
