...
double set_halo_velocities(void)
{
  ...
  for(i=1;i<=N_HALO;i++)
      ...
      if(vdisp_phi<0)
	{
      ...
      double vm2; vm2=2.*vc2_sph_function(sqrt(R*R+z*z)); if(vmax2_halo[i]>vm2) vm2=vmax2_halo[i];
      if((vx*vx+vy*vy+vz*vz)>0.95*vm2)
	{
//	   printf("%d Halo velocity rejected x/y/z=%g/%g/%g ir/iz=%d/%d vx/vy/vz=%g/%g/%g vmax2=%g \n",
//	    i,xp_halo[i],yp_halo[i],zp_halo[i],ir,iz,vx,vy,vz,vmax2_halo[i]);
	  i--;
	}
    }
