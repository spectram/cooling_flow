...
void init(void)
{
    ...
    for(i = 0; i < N_gas; i++)
    {
    double r0 = sqrt(P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]*P[i].Pos[2]);
#ifdef MAGNETIC
    /* initialize radial magnetic fields (not physically most likely, but best-case for quenching */
    for(j = 0; j < 3; j++) {SphP[i].B[j] = 1.0e-7 * P[i].Pos[j] / r0 * gauss2gizmo;}
#endif 
#if defined(GALSF_FB_GASRETURN) || defined(GALSF_FB_RPWIND_LOCAL) || defined(GALSF_FB_HII_HEATING) || defined(GALSF_FB_SNE_HEATING) || defined(GALSF_FB_RT_PHOTONMOMENTUM)
    for(j=0;j<NUM_METAL_SPECIES;j++) {P[i].Metallicity[j]=All.SolarAbundances[j]*(0.1 + 2./(1. + r0/10.));} // appropriate for 1e13 halo
    //for(j=0;j<NUM_METAL_SPECIES;j++) {P[i].Metallicity[j]=All.SolarAbundances[j]*(0.2 + 3./(1. + r0/20.));} // appropriate for 1e14 halo
    /* need to allow for a primordial He abundance */
    if(NUM_METAL_SPECIES>=10) P[i].Metallicity[1]=0.25+(All.SolarAbundances[1]-0.25)*P[i].Metallicity[0]/All.SolarAbundances[0];
#endif    
    }
